...
class Sales extends Secure_area
{
	...
	{	
		...
		if ($this->config->item('track_payment_types') && !empty($track_payment_types)) 
		{
			if ($this->input->post('opening_amount') != '' && !$this->Register->is_register_log_open())  
			{
				...
				$this->Register->insert_register($cash_register,$regiser_payment_data);
	...
	{		
		...
		if($markup_markdown)
		{
			...
			if (isset($markup_markdown_config[$this->input->post('payment_type')]) && $markup_markdown_config[$this->input->post('payment_type')])
			{
				...
				$fee_item = new PHPPOSCartItemSale(array('cart' => $this->cart,'scan' => $fee_item_id.'|FORCE_ITEM_ID|','cost_price' => 0 ,'unit_price' =>$fee_amount ,'description' => '','quantity' => 1));
	...
	function delete_item($item_line)
	{
		...
		{
			$delivery_item_index = $this->cart->get_index_for_delivery_item();
			if($delivery_item_index!== FALSE && $delivery_item_index == $item_line)
			{
				$this->cart->clear_delivery();
			}
	...
	{
		$this->cart->add_paid_store_account_payment_id($sale_id);
	...
	function toggle_pay_all_store_account()
	{
		...
		if(count($this->cart->get_paid_store_account_ids()) ==  0)
		{
			...
			{
					$this->cart->add_paid_store_account_payment_id($unpaid_sale['sale_id']);
